...
class DataListView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if uuid is None:
            return Response(data={'detail': 'uuid is need for get vm Data!'}, status=400)

        if item and item not in ITEM:
            return Response(data={'detail': 'item is not correct!'}, status=400)

        if item and start_time is None:
            return Response(data={'detail': 'start is need for get vm Data if item is specified!'}, status=400)

        try:
            data = mongodb_get_data(uuid, start_time, end_time, item)
        except Exception as error:
            return Response(data={'detail': 'Bad Request!', 'error': str(error)}, status=400)
